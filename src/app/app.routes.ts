...
/**
 * Every feature is lazily loaded. A delivery rider on a phone should not be
 * downloading the admin console, and a browsing visitor should not be
 * downloading the checkout flow before they have put anything in a cart.
 *
...

  {
    ...
    children: [
      ...
      /* ------------------------------ storefront ----------------------------- */
      {
        ...
        loadComponent: () => import('./features/shop/storefront/storefront').then((m) => m.Storefront),
      },
      {
      ...
      /* -------------------------------- buying ------------------------------- */
      {
        ...
        loadComponent: () => import('./features/shop/cart/cart').then((m) => m.CartPage),
      },
      {
        path: 'checkout',
        canActivate: [authGuard],
        title: 'Checkout · Bottle Bee',
        loadComponent: () => import('./features/shop/checkout/checkout').then((m) => m.Checkout),
      },
      {
        path: 'orders',
        canActivate: [authGuard],
        title: 'Your orders · Bottle Bee',
        loadComponent: () => import('./features/shop/orders/order-list').then((m) => m.OrderList),
      },
      {
        path: 'orders/:id',
        canActivate: [authGuard],
        title: 'Order · Bottle Bee',
        loadComponent: () => import('./features/shop/orders/order-track').then((m) => m.OrderTrack),
      },
      {
        path: 'account',
        canActivate: [authGuard],
        title: 'Your account · Bottle Bee',
        loadComponent: () => import('./features/shop/account/account').then((m) => m.Account),
      },
      ...
      /* -------------------------------- admin -------------------------------- */
      {
        ...
        children: [
          ...
          },
          {
            ...
            loadComponent: () => import('./features/admin/vendors/vendors').then((m) => m.AdminVendors),
          },
          {
            path: 'age-verifications',
            title: 'Age verification · Bottle Bee',
            loadComponent: () =>
              import('./features/admin/age-verifications/age-verifications').then(
      ...
      /* -------------------------------- vendor ------------------------------- */
      {
        ...
        children: [
          { path: '', pathMatch: 'full', redirectTo: 'orders' },
          {
            ...
            loadComponent: () => import('./features/vendor/orders/orders').then((m) => m.VendorOrders),
          },
          {
            path: 'products',
            title: 'Your products · Bottle Bee',
            loadComponent: () =>
              import('./features/vendor/products/products').then((m) => m.VendorProducts),
          },
          {
            path: 'inventory',
            title: 'Stock · Bottle Bee',
            loadComponent: () =>
              import('./features/vendor/inventory/inventory').then((m) => m.VendorInventory),
          },
        ],
      },
      ...
      /* ------------------------------- delivery ------------------------------ */
      {
        ...
        children: [
          { path: '', pathMatch: 'full', redirectTo: 'my-deliveries' },
          {
            ...
            loadComponent: () =>
              import('./features/delivery/my-deliveries/my-deliveries').then((m) => m.MyDeliveries),
          },
          {
            path: 'my-deliveries/:id',
            title: 'Delivery · Bottle Bee',
            loadComponent: () =>
              import('./features/delivery/delivery-detail/delivery-detail').then(
                (m) => m.DeliveryDetail,
              ),
          },
        ],
      ...
      /* -------------------------------- errors ------------------------------- */
      {
        ...
        loadComponent: () => import('./features/errors/no-access').then((m) => m.NoAccess),
      },
      {
        path: '**',
